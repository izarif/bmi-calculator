...
return function(loveframes)
  ...
  skin.controls.button_text_nohover_color = { 1, 1, 1, 1 }
  skin.controls.button_text_hover_color = { 1, 1, 1, 1 }
  skin.controls.button_text_nonclickable_color = { 1, 1, 1, 1 }
  skin.controls.button_text_font = skin.controls.smallfont

  -- imagebutton
  skin.controls.imagebutton_text_down_color = { 1, 1, 1, 1 }
  skin.controls.imagebutton_text_nohover_color = { 1, 1, 1, 1 }
  ...
  -- textinput
  skin.controls.textinput_body_color = { 0.961, 0.961, 0.961, 1 }
    ...
    if (tx + twidth) - hwidth / 2 > hx + hwidth then
      if #str > 1 then
        return ParseHeaderText(
          loveframes.utf8.sub(str, 1, #str - 1),
          hx,
          hwidth,
          tx,
          twidth
...
--]]
  ...
  function skin.button(object)
    ...
    if not enabled or not clickable then
      ...
      love.graphics.setColor(1, 1, 1, 1)
      love.graphics.draw(
        skin.images["button-unclickable.png"],
        x,
        y,
        0,
        width,
      ...
      if down or checked then
        ...
        if object.image then
          ...
          love.graphics.draw(
            object.image,
            x + 2,
            y + height / 2 - object.image:getHeight() / 2
          )
          love.graphics.setColor(textdowncolor)
          ...
          while font:getWidth(text) > width - object.image:getWidth() - 10 do
            ...
            while text:byte(1, 1) > 127 do
              text = loveframes.utf8.sub(text, 2)
            end
          end
          skin.PrintText(
            text,
            x + object.image:getWidth() + 4,
            y + height / 2 - theight / 2
          )
        else
          love.graphics.setColor(textdowncolor)
...
--]]
  ...
  function skin.image(object)
    ...
    if stretch then
      scalex, scaley =
        object:GetWidth() / image:getWidth(),
        object:GetHeight() / image:getHeight()
    end
...
--]]
  ...
  function skin.imagebutton(object)
    ...
    if down then
      ...
      skin.PrintText(
        text,
        x + width / 2 - twidth / 2 + 1,
        y + height - theight - 5 + 1
      )
      love.graphics.setColor(textdowncolor)
      skin.PrintText(
        text,
        x + width / 2 - twidth / 2 + 1,
        y + height - theight - 6 + 1
      )
    elseif hover then
      if image then
        love.graphics.setColor(imagecolor)
...
--]]
  ---------------------------------------------------------
  function skin.tabpanel_over(object) end
  ...
  function skin.text(object)
    ...
    for k, v in ipairs(textdata) do
      ...
      if inlist then
        ...
        if
          (y + texty) <= (listy + listhieght)
          and y + (texty + theight) >= listy
        then
          love.graphics.setFont(font)
          if shadow then
            ...
            printfunc(
              text,
              x + textx + shadowxoffset,
              y + texty + shadowyoffset
            )
          end
          if link then
  ...
  function skin.textinput(object)
    ...
    if multiline then
      for i = 1, #lines do
        ...
        skin.PrintText(
          #str > 0 and str or (#lines == 1 and placeholder or ""),
          textx,
...
--]]
  ...
  function skin.columnlistheader(object)
    ...
    local name =
      ParseHeaderText(object:GetName(), x, width, x + width / 2, twidth)
...
--]]
  ...
  function skin.treenode(object)
    ...
    width = width
      + iconwidth
      + loveframes.basicfont:getWidth(object.text)
      + leftpadding
    love.graphics.setColor(1, 1, 1, 1)
